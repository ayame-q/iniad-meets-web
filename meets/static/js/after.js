...
const showCircleInfo = (value=false) => {
	...
	const circlePlayElement = document.getElementById("circle-play")
	...
	if(circleInfo.start_time_str){
		circlePlayElement.classList.remove("nodata")
	} else {
		circlePlayElement.classList.add("nodata")
	}
...
const clearCircleSetTimeOut = () => {
	for(const timeOutID of activeCircleSetTimeOutIds){
		clearTimeout(timeOutID)
	}
}

const setCircleSetTimeOut = (time) => {
	...
	for(const key of Object.keys(circleList)){
		...
		if(circle.start_time_ts && time < circle.video_time_sec){
			activeCircleSetTimeOutIds.push(setTimeout(() => {
				showCircleInfo(circle.id)
			}, (circle.video_time_sec - time) * 1000))
		}
	}
	showCircleInfo(currentCircle.id)
}
...
const connectVideo = () => {
	...
	player.on("durationchange", () => {
		if(!is_video_error){
			const circlePlayElement = document.getElementById("circle-play")
			const circlePlayAElement = circlePlayElement.getElementsByTagName("a")[0]
			circlePlayAElement.addEventListener("click", () => {
				const circlesInfoElement = document.getElementById("circles-info");
				const displayCircleID = circlesInfoElement.value
				player.currentTime(circleList[displayCircleID].video_time_sec)
				if(!is_video_playing){
					player.play()
				}
			})
